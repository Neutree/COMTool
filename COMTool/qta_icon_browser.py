...
class IconBrowser(QtWidgets.QDialog):
    """
    ...
    def _selectIconText(self):
        """
        Select this button, close widget and return value
        """
        indexes = self._listView.selectedIndexes()
        if not indexes:
            return

        self.selected = indexes[0].data()
        self.close()
...
class IconModel(QtCore.QStringListModel):
    ...
    def data(self, index, role):
        ...
        if role == QtCore.Qt.DecorationRole:
            ...
            return qtawesome.icon(iconString, color = self.color)
        return super().data(index, role)

def selectIcon(parent = None, title = 'QtAwesome Icon Browser', btnName = 'OK', color = "gray"):
    browser = IconBrowser(parent, dialog = True, title=title, btnOkName= btnName, color=color)
    browser.show()
    selectedIcon = browser.exec()
    return selectedIcon

def run(dialog = False):
    """
...
if __name__ == '__main__':
    dialog = False
    if len(sys.argv) > 1:
        dialog = sys.argv[1].lower() in ["true", "1"]
    ret = run(dialog = dialog)
    print("ret: ", ret)
